"""Админ-панель — управление каналами, статистика, рассылка
Команды:
...
async def confirm_broadcast(
    ...
    for user_id in user_ids:
        # Повторяем при flood-wait для ЭТОГО ЖЕ юзера
        delivered = False
        while not delivered:
            try:
                await _send_one(user_id)
                delivered = True
                success += 1
            except TelegramRetryAfter as e:
                logger.warning(
                    f"Рассылка: flood-wait {e.retry_after}s, ждём и повторяем для {user_id}"
                )
                await asyncio.sleep(e.retry_after)
                # повтор того же юзера — цикл while
