...
Для текстового бота (мақолы) rate limit применяется ко всем
входящим текстовым сообщениям (не только URL).
"""
import time
import logging
...
# лимит — сколько запросов в минуту
MAX_REQUESTS = 5
WINDOW_SECONDS = 60
...
class RateLimitMiddleware(BaseMiddleware):
    """Ограничивает частоту запросов — только для текстовых сообщений"""
    ...
    async def __call__(
        ...
        # пропускаем только /start (чтобы юзер всегда мог открыть бота).
        # Остальные команды (включая /admin, /random, /today, /favorites) — лимитируем.
